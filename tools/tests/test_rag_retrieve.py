...
def test_exclude_ids_reaches_the_vector_store_filter(monkeypatch):
    ...
    assert captured["max_num_results"] == rr.TITLE_RERANK_CANDIDATE_K
...
def test_the_article_named_by_the_query_comes_first(monkeypatch):
    """Asked for `navigator`, the store put the navigator article at rank 11 of
    the branch — behind longer articles that merely talk about navigators. Its
    title says what it is, and a full match is worth more than the 0.14 the
    store separated them by."""
    _patch_vs_search(monkeypatch, _FakeResp([
        _vshit("Rules_sessions::x", "Rules: change sessions > Session rules", 0.63),
        _vshit("Rules_navigator::navigator-rules", "Rules: navigator > Navigator rules", 0.49),
    ]))
    hits = rr._vs_search_for_source("navigator", "rules", 7)
    assert [h.id for h in hits][0] == "Rules_navigator::navigator-rules"
    assert hits[0].score == 0.49  # score stays the store's own number


def test_a_query_worded_differently_leaves_the_order_alone(monkeypatch):
    """The rule fires on an exact whole-word title match and nothing else, so a
    question phrased in the reader's words — the case the vector search is good
    at — is returned exactly as the store ranked it."""
    _patch_vs_search(monkeypatch, _FakeResp([
        _vshit("Rules_sessions::x", "Rules: change sessions > Session rules", 0.79),
        _vshit("Rules_navigator::navigator-rules", "Rules: navigator > Navigator rules", 0.49),
    ]))
    hits = rr._vs_search_for_source("application menu tree the user starts from", "rules", 7)
    assert [h.id for h in hits] == ["Rules_sessions::x", "Rules_navigator::navigator-rules"]


def test_one_extra_word_does_not_get_past_a_decisively_better_hit(monkeypatch):
    """A partial match is a guess. It is worth a nudge, not the top spot: here
    the store put another article 0.30 ahead, which a half-covered title
    (0.5 * 0.3) cannot close."""
    _patch_vs_search(monkeypatch, _FakeResp([
        _vshit("Rules_sessions::x", "Rules: change sessions > Session rules", 0.79),
        _vshit("Rules_navigator::navigator-rules", "Rules: navigator > Navigator rules", 0.49),
    ]))
    hits = rr._vs_search_for_source("navigator caching", "rules", 7)
    assert [h.id for h in hits][0] == "Rules_sessions::x"


def test_a_query_of_nothing_but_stopwords_promotes_nothing(monkeypatch):
    """`wanted` is empty, and an empty set is a subset of every title — without
    the guard this would promote the whole response."""
    _patch_vs_search(monkeypatch, _FakeResp([
        _vshit("Rules_sessions::x", "Rules: change sessions > Session rules", 0.79),
        _vshit("Rules_navigator::navigator-rules", "Rules: navigator > Navigator rules", 0.49),
    ]))
    hits = rr._vs_search_for_source("the rules for", "rules", 7)
    assert [h.id for h in hits] == ["Rules_sessions::x", "Rules_navigator::navigator-rules"]


def test_a_russian_query_survives_tokenisation_and_matches_nothing(monkeypatch):
    """Titles are English. A Russian query must fall back to the store's order —
    not be stripped to whatever Latin token it happens to contain and match on
    that."""
    _patch_vs_search(monkeypatch, _FakeResp([
        _vshit("Rules_sessions::x", "Rules: change sessions > Session rules", 0.79),
        _vshit("Rules_navigator::navigator-rules", "Rules: navigator > Navigator rules", 0.49),
    ]))
    hits = rr._vs_search_for_source("правила навигатора", "rules", 7)
    assert [h.id for h in hits] == ["Rules_sessions::x", "Rules_navigator::navigator-rules"]
    assert rr._words("правила навигатора") == {"правила", "навигатора"}


def test_a_title_word_inside_a_longer_word_does_not_count(monkeypatch):
    """Whole words only: `form` must not match `platform`."""
    _patch_vs_search(monkeypatch, _FakeResp([
        _vshit("Rules_a::x", "Rules: information > Information rules", 0.9),
        _vshit("Rules_b::y", "Rules: platform > Platform rules", 0.4),
    ]))
    hits = rr._vs_search_for_source("form", "rules", 7)
    # `form` is inside `platform` and `Information`, and matches neither.
    assert [h.id for h in hits] == ["Rules_a::x", "Rules_b::y"]


def test_a_hit_without_a_heading_path_is_simply_not_promoted(monkeypatch):
    """A stale or foreign file carries no title. That is not an error, it just
    cannot be title-matched — so it keeps its bare score while the named
    article collects the bonus and passes it."""
    _patch_vs_search(monkeypatch, _FakeResp([
        _FakeVSHit({"section_id": "Foreign::x"}, score=0.6),
        _vshit("Rules_navigator::navigator-rules", "Rules: navigator > Navigator rules", 0.4),
    ]))
    hits = rr._vs_search_for_source("navigator", "rules", 7)
    assert [h.id for h in hits][0] == "Rules_navigator::navigator-rules"


def test_the_promotion_survives_the_cross_branch_merge(monkeypatch):
    """Merging five branches by score alone would undo it the moment it is
    needed most — an untyped search."""
    def fake_search(**kwargs):
        branch = kwargs["filters"]["value"] if kwargs["filters"]["type"] == "eq" else \
                 kwargs["filters"]["filters"][0]["value"]
        if branch == "rules":
            return _FakeResp([_vshit("Rules_navigator::navigator-rules",
                                   "Rules: navigator > Navigator rules", 0.40)])
        return _FakeResp([_vshit(f"{branch}::x", f"{branch}: something else", 0.60)])

    monkeypatch.setattr(rr.client.vector_stores, "search", fake_search)
    out = rr.retrieve_docs_tool("navigator")
    assert out.docs[0].id == "Rules_navigator::navigator-rules"

def test_a_partial_title_match_overtakes_a_near_neighbour(monkeypatch):
    """The point of the bounded nudge: naming an article and one thing besides
    ("navigator caching") should not drop the caller back to no help at all.
    Here the store separated the two by 0.05, and half a title closes that."""
    _patch_vs_search(monkeypatch, _FakeResp([
        _vshit("Rules_sessions::x", "Rules: change sessions > Session rules", 0.50),
        _vshit("Rules_navigator::navigator-rules", "Rules: navigator > Navigator rules", 0.45),
    ]))
    hits = rr._vs_search_for_source("navigator caching", "rules", 7)
    assert [h.id for h in hits][0] == "Rules_navigator::navigator-rules"
    assert hits[0].score == 0.45  # the nudge ranks, it does not touch the score


def test_a_full_match_still_cannot_pass_a_decisively_better_hit(monkeypatch):
    """The bonus is bounded, and that is the whole defence against a generic
    word. `operator` is a full match for a SECTION named "Operator" — which is
    what dozens of language articles call one of their sections. An earlier
    version gave every full match a tier of its own and all of them jumped the
    hit the store was sure about; a bounded addition cannot."""
    _patch_vs_search(monkeypatch, _FakeResp([
        _vshit("Language_aggr::x", "AGGR operator > Syntax", 0.99),
        _vshit("Language_other::y", "Something unrelated > Operator", 0.10),
    ]))
    hits = rr._vs_search_for_source("operator", "language", 7)
    assert [h.id for h in hits][0] == "Language_aggr::x"


def test_top_score_is_the_best_store_score_not_the_first_hit_s(monkeypatch):
    """The list is ranked by score PLUS the heading bonus, so its head need not
    be the best-scoring chunk. Gap analytics compare stores, not orders — a
    `top_score` taken from rank 1 would report a false gap the moment the bonus
    reorders anything."""
    calls = _capture_emit(monkeypatch)
    _patch_vs_search(monkeypatch, _FakeResp([
        _vshit("Rules_sessions::x", "Rules: change sessions > Session rules", 0.62),
        _vshit("Rules_navigator::navigator-rules", "Rules: navigator > Navigator rules", 0.49),
    ]))
    out = rr.retrieve_docs_tool("navigator", type="rules")
    assert out.docs[0].id == "Rules_navigator::navigator-rules"   # bonus moved it up
    assert out.docs[0].score == 0.49                              # its own score is lower
    assert calls[0]["fields"]["top_score"] == 0.62                # and top_score is the max


def test_coverage_is_the_share_of_the_query_the_headings_carry():
    """The numbers the ranking runs on, stated once and directly."""
    # Every meaningful word in the article's own title.
    assert rr._title_coverage("navigator", "Rules: navigator > Navigator rules", "Rules_navigator") == 1.0
    # Only in a section: the reduced weight, not the full one.
    assert rr._title_coverage("migration", "Rules: sessions > Migration notes", "Rules_sessions") == 0.4
    # Half the query named, half of it nowhere — the denominator keeps BOTH.
    assert rr._title_coverage("navigator caching", "Rules: navigator > Navigator rules", "Rules_navigator") == 0.5
    # Words no heading carries stay in the denominator. This is what keeps a
    # paraphrase honest: one word in four, not "all the words I happen to know".
    assert rr._title_coverage("forbid saving invalid data", "Rules: data export > Shaping the result", "Rules_export") == 0.25
...
def test_a_word_in_both_the_title_and_a_section_counts_once():
    ...
    assert rr._title_coverage("rules navigator", "Rules: navigator > Navigator rules", "Rules_navigator") == 1.0


def test_coverage_edge_cases_do_not_raise():
    assert rr._title_coverage("", "Rules: navigator > Navigator rules", "Rules_navigator") == 0.0
    assert rr._title_coverage("the rules for", "Rules: navigator > Navigator rules", "Rules_navigator") == 0.0
    # No heading path at all: the slug is still the article's own name.
    assert rr._title_coverage("navigator", "", "Rules_navigator") == 1.0
    assert rr._title_coverage("navigator", "", "") == 0.0
    # A heading with no " > " is all article title.
    assert rr._title_coverage("navigator", "Rules: navigator", "") == 1.0


def test_a_russian_query_matches_a_russian_heading():
    """The old test only proved Russian does not vanish. This one proves it can
    match — and that a decomposed `й` compares equal to the composed one."""
    assert rr._title_coverage("навигатор", "Правила: навигатор > Правила навигатора", "") == 1.0
    decomposed = "наи\u0306\u0442и"          # й as и + combining breve
    composed = "найти"
    assert rr._words(decomposed) == rr._words(composed)


def test_a_full_title_match_cannot_cross_a_gap_wider_than_the_bonus(monkeypatch):
    """The bound, stated on a real full match: the query names this article and
    nothing else, and it still does not pass a hit the store put 0.4 ahead."""
    _patch_vs_search(monkeypatch, _FakeResp([
        _vshit("Rules_sessions::x", "Rules: change sessions > Session rules", 0.90),
        _vshit("Rules_navigator::navigator-rules", "Rules: navigator > Navigator rules", 0.50),
    ]))
    hits = rr._vs_search_for_source("navigator", "rules", 7)
    assert [h.id for h in hits][0] == "Rules_sessions::x"


def test_a_section_match_is_worth_less_than_a_title_match(monkeypatch):
    """A title says what the article is about; a section name is a detail of
    it. Same word, same score — the article whose TITLE carries it wins."""
    _patch_vs_search(monkeypatch, _FakeResp([
        _vshit("Rules_a::x", "Rules: sessions > Migration notes", 0.50),
        _vshit("Rules_migration::y", "Rules: migration > Rules", 0.50),
    ]))
    hits = rr._vs_search_for_source("migration", "rules", 7)
    assert [h.id for h in hits][0] == "Rules_migration::y"
